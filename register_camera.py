...
def PnP(config, crn_match, camera_2d, camera_3d, min_points=4, min_inliers=4):
    """
    RANSAC PnP. Returns (camera_params, indice_points_2d, indice_points_3d)
    or (None, None, None) if not solvable / not enough correspondences.
    """
    ...
    # ---- convert / build arrays ----
    if len(indice_points_3d) < min_points:
        return None, None, None

    new_points_2d = np.asarray(new_points_2d, dtype=np.float64)
    existing_points_3d = np.asarray(
        config.reconstructed_points_3d[indice_points_3d],
        dtype=np.float64
    )

    # OpenCV expects Nx1x2 or Nx2 for image points; Nx3 for object points
    if existing_points_3d.shape[0] < min_points or new_points_2d.shape[0] < min_points:
        return None, None, None
    if existing_points_3d.shape[0] != new_points_2d.shape[0]:
        return None, None, None

    # ---- solvePnPRansac with guards ----
    ok, rvec, tvec, ind_inliers = cv.solvePnPRansac(
        objectPoints=existing_points_3d,
        imagePoints=new_points_2d,
        cameraMatrix=config.K,
        distCoeffs=config.DISTCOEFFS,
        reprojectionError=float(config.pnp_threshold),
        iterationsCount=200,
        confidence=0.99,
        flags=cv.SOLVEPNP_ITERATIVE
    )

    if (not ok) or (ind_inliers is None) or (len(ind_inliers) < min_inliers):
        return None, None, None

    ind_inliers = ind_inliers.ravel()

    camera_params = np.hstack((rvec.flatten(), tvec.flatten())).astype(np.float64)

    indice_points_2d = np.asarray(indice_points_2d, dtype=np.int64)[ind_inliers]
    indice_points_3d = np.asarray(indice_points_3d, dtype=np.int64)[ind_inliers]
...
def register(config, dict_cameras):
    ...
    # ---- If PnP not solvable (too few points / too few inliers), skip cleanly ----
    if camera_param is None:
        # Mark this edge as unusable so next_camera() doesn't keep selecting it
        try:
            config.n_good_matches[index_camera_3d, index_camera_2d] = 0
        except Exception:
            pass

        print(
            f"PnP skipped: insufficient correspondences to register camera {index_camera_2d} "
            f"by camera {index_camera_3d} (pairs={n_pairs})."
        )
        return config, dict_cameras, 0, 0
    ...
    print(
        'Use %d out of %d points to register camera %d by camera %d'
    ...
    for index_camera in tmp:
        ...
        dict_cameras[index_camera_2d], dict_cameras[index_camera], config = reconstruct_3d_and_link_2d(
            config, crn_match, P1, P2, camera_new, camera_exist
        )
    ...
    # optimize (GUARDED residuals: keep least_squares from failing on NaN/inf)
